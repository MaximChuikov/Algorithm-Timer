    ...
    public void BogoSort(int[] array)
    {
        ...
        void Shuffle(int[] arr)
        {
            for (int i = 0; i < arr.Length; ++i)
                Swap(arr, i, rand.Next(0, arr.Length));
        }
...
static class BackPackTask
{
    public static int BackPack(Tuple<int,int>[] weightCost, int maxWeight)
    {
        List<Tuple<int,int>> graphLeafs = new();
        int endArr = weightCost.Length - 1;
        RecChoose(0, 0, 0);
        return graphLeafs.Where(l => l.Item1 <= maxWeight).Max(l => l.Item2);

        void RecChoose(int currentCost, int currentWeight, int i)
        {
            if (i == endArr)
                graphLeafs.Add(new Tuple<int,int>(currentWeight, currentCost));
            else
            {
                RecChoose(currentCost + weightCost[i].Item2, currentWeight + weightCost[i].Item1, i + 1); //приняли
                RecChoose(currentCost, currentWeight, i + 1); //не приняли
            }
        }
    }
}
static class BinarySearchTask
{
    public static int BinarySearch(int[] arr, int key) => RecBinSearch(arr, key, 0, arr.Length - 1);
    private static int RecBinSearch(int[] arr, int key, int leftEdge, int rightEdge)
    {
        int mid = (leftEdge + rightEdge) / 2;
        if (arr[mid] == key)
            return mid;
        else if (arr[mid] < key) 
            return RecBinSearch(arr, key, mid + 1, rightEdge);      //то вправо
        else
